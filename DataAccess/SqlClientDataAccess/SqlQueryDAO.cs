...
namespace Estar.DataAccess.SqlClientDataAccess
{
	...
	public class SqlQueryDAO:IQueryDAO
	{
        ...
        private static Boolean isCache = true;			//数据资源文件
        private static XmlDocument xmldocResource = new XmlDocument();
        /// <summary>
        /// 缓存单元配置
        /// </summary>
        public static Hashtable UnitHash = new Hashtable();
        /// <summary>
        /// 缓存配置数据
        /// </summary>
        private static Dictionary<string, DataTable> tabsCache = new Dictionary<string, DataTable>();
        /// <summary>
        /// 是否采用Cache模式
        /// </summary>
        ...
        {
            get { return isCache; }
            set { isCache = value; if (!isCache) { xmldocResource.RemoveAll(); UnitHash.Clear(); };}
        }
        ...
        {
            if (isCache && tabsCache.ContainsKey(strsql))
                return tabsCache[strsql];
            ...
            {
                da.Fill(dt);
                if (!tabsCache.ContainsKey(strsql))
                    tabsCache.Add(strsql, dt);
                return dt;
            }
            catch
            {
        ...
        protected void NodeCascade(XmlNode lxn_node, string[] strParams)
        {
            ...
            foreach (XmlNode lxn_parameter in lxnl_param)
            {
                ...
                if (li_len > 500 && SqlDbType.VarChar == lty_oratype)
                {
                    lty_oratype = SqlDbType.Text;
                    li_len = -1;
                }
                //数据版本
		...
		protected	bool	ExecuteNonQuery(string item,string ps_optype,NameObjectList	parameters)
		{
			...
			//初始化数据项目的规则
            Monitor.Enter(this._conn);
			if(item!=this._item)
				this.SetItem(item);
            if (this._item.Length < 1)
            {
                Monitor.PulseAll(this._conn);
                Monitor.Exit(this._conn);
                return false;
            }
            ...
            try
			{
				...
				for(int j=0;j<listCmdCascade.Count;j++)
				{
					SqlCommand	cmdtemp=listCmdCascade[j] as SqlCommand;
					if(cmdtemp==null)	continue;
                    cmdtemp.Transaction = this._trans;
        ...
        private XmlNode setXmlDataItem(string _dataitem)
        {
            ...
            if (IsCache == true)
            {
                xnItem = SqlQueryDAO.xmldocResource.SelectSingleNode(string.Format("/*/DataItem[@name='{0}']", item.Replace("'","&apos;")));
                if (null != xnItem) return xnItem;
            }
            //从缓存获取 结束

            //读取数据库配置生成数据项配置节点
            string strXml = "<DataItem name='" + item.Replace("'", "&apos;") + "'></DataItem>";
            XmlDocument myXmldoc = new XmlDocument();
            myXmldoc.LoadXml(strXml);
            DataTable tabitem = CreateDataTable(item);
            myXmldoc = CreateXmlNode(myXmldoc, tabitem);
        ...
        private XmlDocument CreateXmlNode(XmlDocument myxmldoc, DataTable tabitem)
        {
            if (null == tabitem || tabitem.Rows.Count < 1)
                return myxmldoc;
            DataRow[] drUnit = tabitem.Select("ntype='DataItem'");
            ...
            DataRow[] drItem = tabitem.Select("ntype='SQLText'");
            if (drItem != null)
            {
        ...
        private DataTable CreateDataTable(string item)
        {
        ...
        private bool isConn
        {
            get { return ConnectionState.Open == this._conn.State || ConnectionState.Fetching == this._conn.State || ConnectionState.Executing == this._conn.State; }
        }
        /// <summary>
        /// 启动数据库连接
        /// </summary>
        ...
        public bool Open()
        {
            if (this.isConn)
            {
                if (null == this._trans || null == this._trans.Connection || this._conn != this._trans.Connection)
                    this._trans = null;
                return true;
            }
            try
		...
		public bool		RollbackAndClose()
		{
			try
			{
                if (null == this._trans || null == this._trans.Connection || this._conn != this._trans.Connection)
                    this._trans = null;
				if(this.isConn && null!=this._trans)
		...
		public bool		Close()
		{
			try
			{
                try
                {
                    if (null == this._trans || null == this._trans.Connection || this._conn != this._trans.Connection)
                        this._trans = null;
                    if (this.isConn && this._trans != null)
		...
		public DataTable GetDataTable(string item, NameObjectList parameters)
		{
            ...
            if (item != this._item)
                this.SetItem(item);
            if (string.IsNullOrEmpty(this._item) || null == this._adapter.SelectCommand)
            {
                Monitor.PulseAll(this._conn);
                Monitor.Exit(this._conn);
                return null;
		...
		public	int			FillDataSet(string item, NameObjectList parameters,DataSet ds)
		{
            ...
            if (item != this._item)
                this.SetItem(item);
            if (this._item.Length < 1 || null == this._adapter.SelectCommand)
            {
                Monitor.PulseAll(this._conn);
                Monitor.Exit(this._conn);
                return irtn;
		...
		public bool		ExecuteNonQuery(string item,NameObjectList paramsIns,NameObjectList paramsUpt,NameObjectList paramsDel)
		{
			...
			catch ( Exception ex )
			{
                ...
                errInfo["数据源"] = this._item;
                errInfo["数据文件"] = this._xmlfile;
                errInfo["时间"] = Convert.ToString(paramsIns["时间"]);
                ExceptionManager.Publish(ex, errInfo);
                this.RollbackAndClose();
                if (!isauto)
                {
                ...
                if (ex is SqlException && 1205 == ((SqlException)ex).Number)
                {
                    this._conn = new SqlConnection(this._conn.ConnectionString);
                    SqlCommand[] cmds ={ this._adapter.SelectCommand, this._adapter.UpdateCommand, this._adapter.DeleteCommand };
                    for (int m = 0; m < 3; m++)
                    {
                        if (null != cmds[m])
                            cmds[m].Connection = this._conn;
                    }
                }
                isSuccess = false;
			}
		...
		public bool		ExecuteNonQuery(string item,NameObjectList[] paramsListIns,NameObjectList[] paramsListUpt,NameObjectList[] paramsListDel)
		{
            ...
            bool isSuccess=true;	//默认不使用外部定制连接事务,默认成功执行
            bool isauto = !this.isConn;
			try
			{   
				//打开数据库，提交数据
			...
			catch ( Exception ex )
			{
                ...
                errInfo["数据源"] = this._item;
                errInfo["数据文件"] = this._xmlfile;
                errInfo["SQL="] = this.UptCommand.CommandText;
                ExceptionManager.Publish(ex, errInfo);
                this.RollbackAndClose();
                if(!isauto)
                {
