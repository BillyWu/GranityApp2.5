...
namespace Estar.DataAccess.OleDbDataAccess
{
	/// <summary>
	/// OLEDB方式数据访问
	/// </summary>
	public class OleDbQueryDAO:IQueryDAO
	{
		private OleDbConnection		_conn;		//数据操作的数据库连接
		private OleDbDataAdapter		_adapter;	//操作数据的数据适配器:包含自身的select,insert,update,delete规则
		private OleDbDataAdapter		_adpCascade;	//级联操作的数据适配器,更新规则来源两个command列表
		private OleDbTransaction	_trans;			//外部定制方式启动的数据库事务
		...
		public OleDbQueryDAO()
		{
			_xmldoc=new XmlDocument();
			_conn=new OleDbConnection();
			_adapter=new OleDbDataAdapter("",_conn);
			_adpCascade=new OleDbDataAdapter("",_conn);
			_adpCascade.RowUpdated +=new OleDbRowUpdatedEventHandler(OnRowUpdated);
		...
		protected	void	NodeCascade(XmlNode lxn_node,string[] strParams)
		{
			...
			OleDbCommand cmd=new OleDbCommand(ls_sql,_conn);
			...
			foreach(XmlNode lxn_parameter in lxnl_param)
			{
                ...
                lty_oratype = OleDbType.VarChar;
				//参数类型
				switch(ls_datatype.ToLower())
				{
					case "date":
						lty_oratype=OleDbType.Date;break;
					case "number":
					case "float":
					case "real":
						lty_oratype=OleDbType.Decimal;break;
					case "int":
						lty_oratype=OleDbType.Integer;break;
					case "varchar2":
					case "varchar":
					case "string":
						lty_oratype=OleDbType.VarChar;break;
					case "char":
						lty_oratype=OleDbType.Char;break;
					case "uniqueidentifier":
					case "guid":
						lty_oratype=OleDbType.Guid;break;
				}
				//数据版本
				...
				if (ls_srccolumn.Length>0)
				{
					if(li_len>0)
					{
						lp_param=new OleDbParameter(ls_name,lty_oratype,li_len,ls_srccolumn);
						lp_param.SourceVersion=lvs_version;
					}
					else
					{
						lp_param=new OleDbParameter(ls_name,lty_oratype);
						lp_param.SourceColumn=ls_srccolumn;
						lp_param.SourceVersion=lvs_version;
					}
				}
				else if(li_len>0)
					lp_param=new OleDbParameter(ls_name,lty_oratype,li_len);
				else
					lp_param=new OleDbParameter(ls_name,lty_oratype);
		...
		{
			string	ls_key;
			OleDbCommand		cmd=null;		//用于操作的sql命令
			ArrayList			listCmdCascade;	//级联的操作命令列表
			OleDbParameter		param;			//命令用到的参数
			OleDbTransaction	trans=null;			//事务控制
			//初始化数据项目的规则
			...
			try
			{
				...
				{
					OleDbCommand	cmdtemp=listCmdCascade[j] as OleDbCommand;
        ...
        public object ExecuteScalar(string item, NameObjectList parameters)
        {
            ...
            OleDbCommand cmd = this._adapter.SelectCommand;
        ...
        public DataTable GetDataTableBySql(string strSQL)
        {
            ...
            this._item = "";
            if (null == this._adapter.SelectCommand)
                this._adapter.SelectCommand = new OleDbCommand(strSQL, this._conn);
            else
		...
		public	int			FillDataSet(string item, NameObjectList parameters,DataSet ds)
		{
			...
			try
			{
				...
				{
					ls_key=parameters.Keys[i];
					if(!cmd.Parameters.Contains(ls_key))
						continue;
					param=cmd.Parameters[ls_key];
					if(param==null) continue;		//传入没有的参数时忽略
					param.Value=parameters[ls_key];
		...
		public bool		Update(string item,DataTable tab)
		{
			...
			try
			{
				...
				{
					cmd=this._listCmdCasInsert[i] as OleDbCommand;
				...
				{
					cmd=this._listCmdCasUpdate[i] as OleDbCommand;
					if(cmd==null)	continue;
					cmd.Transaction=trans;
				...
				{
					cmd=this._listCmdCasDelete[i] as OleDbCommand;
					if(cmd==null)	continue;
					cmd.Transaction=trans;
			...
			{	//传递参数
				...
				for(int j=0;j<listCmdCascade.Count;j++)
				{
					...
					for(int k=0;k<parameters.Count;k++)
					{
						...
						OleDbParameter		param=cmdtemp.Parameters[ls_key];
						if(param==null)	continue;
						param.Value=parameters[ls_key];
					}
