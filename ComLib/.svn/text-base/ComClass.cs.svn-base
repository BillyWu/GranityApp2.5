...
namespace ComLib
{
    ...
    public class ComClass
    {
                            ...
                            for (int m = 0; m < subs.Length; m++)
                            {
                                ...
                                switch (subDataType.ToLower())
                                {
                                    ...
                                    case "int":
                                        if (string.IsNullOrEmpty(subValue))
                                            subValue = "0";
                                        subValue = Convert.ToInt32(Convert.ToInt32(subValue) * pmobj.Scale).ToString();
                                        subValue = sVal10To2(subValue, iSublen);
                                        break;
                                    case "bit":
                                        if (string.IsNullOrEmpty(subValue))
                                            subValue = "0";
                                        subValue = subValue.PadLeft(iSublen, '0');
                                        break;
                                    default:
                                        subValue = subValue.PadLeft(iSublen, '0');
                                        break;
        ...
        {
            if (string.IsNullOrEmpty(data) || string.IsNullOrEmpty(srcsign))
                return data;
            string srcL = srcsign.ToLower();
            string destL = destsign.ToLower();
            string srcU = srcsign.ToUpper();
            string destU = destsign.ToUpper();
            if (!data.Contains(srcL) && !data.Contains(srcU))
                return data;
            //末尾一个字节
            string srcLw = srcL.Substring(srcL.Length - 2, 2);
            string srcUw = srcU.Substring(srcU.Length - 2, 2);
            int k = srcsign.Length;
            for (int i = data.Length - 2; i > k-3; i -= 2)
            {
                string s = data.Substring(i, 2);
                if (srcLw == s)
                {
                    if (srcL == data.Substring(i - k + 2, k))
                    {
                        data = data.Substring(0, i - k + 2) + destL + data.Substring(i + 2);
                        i -= k - 2;
                    }
                }
                else if (srcUw == s)
                {
                    if (srcU == data.Substring(i - k + 2, k))
                    {
                        data = data.Substring(0, i - k + 2) + destU + data.Substring(i + 2);
                        i -= k - 2;
                    }
                }
            }
        ...
        public string getValByHmFormat(string formats, string[] udpinfos, devObj devobj, Boolean noHE, ref string vtag)
        {
                        ...
                        if (pmobj.SubItems != "")
                        {
                            ...
                            for (int m = 0; m < subs.Length; m++)
                            {
                                ...
                                switch (subDataType.ToLower())
                                {
                                    ...
                                    case "integer":
                                    case "int":
                                        subValue = Convert.ToInt32(Convert.ToInt32(subValue, 2) * pmobj.Scale).ToString();
                                        break;
                                    default:
                                        subValue = Convert.ToInt32(subValue, 2).ToString();
                                        break;
            ...
            //门禁时有返回则成功
            if (!string.IsNullOrEmpty(rtns) && string.IsNullOrEmpty(basefun.valtag(rtns, "Success")))
                rtns = basefun.setvaltag(rtns, "Success", "true");
            if (rtn.Length > 0)
                rtn = rtn.Substring(1);
            rtns = rtns.Trim();
