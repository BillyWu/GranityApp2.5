...
function createHint()
{
    ...
    for(var i=0;i<rowcount;i++)
    {
		...
		if(bartitle!="" && refAdd.mtype!="frame")
		{
		    switch(xstate)
		    {
		        case "1":
	    ...
	    else
	    {
	        if(refAdd.mtype!="frame")
	        {
                ...
                refAdd.itemcolor="gray";
            }
	    }
		
        refAdd.title=bartitle;
        var xleft = refAdd.style.posLeft;
		...
		switch(strhint)
		{
		    ...
		    case "0":
        	    break;
        	default:
        		create_text(band.getFldStrValue("销量",i).replace("次",""),xleft,xtop,12,obj_zIndex,refAdd.id);
		}
    }
}
...
function create_text(val,vleft, vtop, vsize,obj_zIndex,objId)
{
	...
	{
	    var oshape = grpObj.appendChild(document.createElement('<v:shape id='+objId+'shape'+' style="Z-INDEX:'+obj_zIndex+';LEFT:'+ vleft +'px;WIDTH:'+document.getElementById(objId).style.width+';TOP:'+ vtop +'px;HEIGHT:20px" inset="1px,1px,1px,1px" coordsize="1000,680"  filled="f" >'));
	    var oText = oshape.appendChild(document.createElement('<DIV id='+objId+'text'+' style="font-family: 宋体;FONT-SIZE: 8pt; CURSOR: hand; COLOR: black; TEXT-ALIGN: center">'));
 	}
 	else
...
function mapopenwin(obj,pitem,item)
{
    ...
    {
        var mapname = band.getFldStrValue("地名",i);
        if(mapname==obj.id)
        {
