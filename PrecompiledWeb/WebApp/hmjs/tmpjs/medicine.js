  ...
  var lcolor = 3; //颜色码长度
  var BarHtml="";
  var elems1 = ["药品资料","视图","拆零","操作方法","促销发布","保存","返回"];
  ...
  var IsModify = false;
  var SearchFields = "款号;款式;dbo.fun_getPY(名称);名称;失效" ;
  function initWin()
  {
    gband=$band("goods");
    InitView();
    var scope = (!$U().Scope)?(($U().Rights=="")?"部门":$U().Rights):$U().Scope;
    $("supplier").datasourceid = $("supplier").datasourceid.replaceAll("@Scope","'"+scope+"'");
    $("supplier").value=$SP("DWName");
    $("supplier").tag="@主代码="+$SP("UnitCode");
    if($U().Right=="增删改") {IsModify = true;}
    usBeforeFsearch();
    secBoard(0);
    gband.editdict=true;
    $loading("none");
...
function secBoard(n) 
{
    ...
    switch(n)
    {
        case 0:
            if(!gband) return;
            gband.minwidth = "70px";    gband.gridtype = 0;    gband.freecols = "名称";
            gband.Asyn = true;
            if(!IsModify) {
        ...
        case 2:
            ...
            $("pubkh").innerText="【"+gband.Val("名称")+"-"+gband.Val("编码")+"】";
            break;        
    }    
}
...
function ts_Afteradd(ob)
{
    ...
    ob.setFldStrValue(null,"公司代码", ToolUtil.valueTag($("supplier").tag,"主代码"));
    openPro();
    return true;
}
...
function cboAfterUpdate(o)
{
    ...
    //处理编辑状态
    if((ToolUtil.valueTag($("supplier").tag,"主代码")==$SP("UnitCode") || $U().Rights=="全部") && $U().Right=="增删改")
    {IsModify = true;$("Btn保存").style.display="block";gband.noxml=false;gband.gridtype=0;}
    else {$("Btn保存").style.display="none";IsModify = false;gband.noxml=true;gband.gridtype=6;};
    $('btnfSearch').fireEvent('onclick');
}    
function InitView()
{
    var xmlhttp = ue_ajaxdom("exec FD_商品大类",null,null,null,null,true);
       ...
       if(_x == 4 && xmlhttp.status == 200)
        {
            ...
            elems2.push("全部");elemsevent2.push("ue_pfilter('goods')");
            ...
            elems2.push("失效品");elemsevent2.push("ue_pfilter('goods','失效=1')");
            //elems2.push("更多...");elemsevent2.push("Fcbo('goods','exec FD_商品类别|name|title')");            
...
{
    $loading();
     $TP("药类", event.srcElement.innerText,gband.ItemName);
    $('btnfSearch').fireEvent('onclick');
}    
...
function dlgsplit()
{
    ...
    var s= '<fieldset style="padding: 2px;width:99%;height:50px;text-align:left"><legend><font face="微软雅黑">主药品信息：</font></legend>\
            <span class="span60fn">药品编码：</span><span datasrc="#商品资料Tab" datafld="编码" class="xlandinput" style="width:60px;"></span>\
...
function strfind1(winid, gridname, srcitem, destitem,callback,mitem,spos)
{
    ...
    + storeinfo
    +'        <td width="60" align="right">药品编码:</td><td>'
    +'        <input id=\"款号\" name=\"编码\" filter=\"and\" class=\"xlandinput\" size=\"1\" style=\"WIDTH: 100%;\"></td>'
...
function openPro()
{
    ...
    YPanel("goods","dvCustTab",null,1,"@零售价格=3",150,70);
...
function strfind(winid,gridname,bandname)
{
    ...
    if(!IsModify)
        var s= '<fieldset style="padding: 2px;;width:99%"><legend><font face="微软雅黑">主要信息：</font></legend>\
            ...
            <td align="right" width="70">&nbsp;药品名称：</td><td width="235">\
            <span datasrc="#商品资料Tab" datafld="名称" class="spanbox" style="WIDTH: 90%;" ></span><font face="Wingdings" color="#ff0000">v</font></td>\
            <td align="right" width="60">药品编码：</td><td width="100">\
            <span datasrc="#商品资料Tab" datafld="编码" class="spanbox" style="width:80%;" ></span><font face="Wingdings" color="#ff0000">v</font></td>\
        ...
        var s= '<fieldset style="padding: 2px;;width:99%"><legend><font face="微软雅黑">主要信息：</font></legend>\
            ...
            <td align="right" width="60">药品编码：</td><td width="100">\
            <input datasrc="#商品资料Tab" datafld="编码" class="xlandinput" style="width:80%;" type="text"  /><font face="Wingdings" color="#ff0000">v</font></td>\
            <td align="right" width="40">失效：</td><td width="20">\
    ...
    var s1='<div style="width:99%;height:390px">\
        <fieldset style="float:left;padding: 2px;height:220;width:69%"><legend><font face="微软雅黑">详细资料：</font></legend>\
        <div id="dvCustTab" style="height:198px;width:100%;text-align:left"></div>\
        </fieldset>\
        <fieldset style="float:right;padding: 2px;height:220;width:29%"><legend><font face="微软雅黑">图标：</font>';
    if(IsModify)
    ...
    var s3='</legend>\
        ...
        <fieldset style="float:left;padding: 2px;height:85;width:69%"><legend><font face="微软雅黑">GSP属性：</font></legend><div id="dvSubTab" style="width:100%;height:95%;text-align:left"></div></fieldset>\
        <fieldset style="float:right;padding: 2px;height:160;width:29%"><legend><font face="微软雅黑">功能主治：</font></legend>'+'<textarea name="S1"  class="xlandinput" datasrc="#商品资料Tab" datafld="功能" style="width:98%;height:98%" ></textarea></fieldset>\
        <fieldset style="float:left;padding: 2px;height:40;width:69%"><legend><font face="微软雅黑">其它属性：</font></legend>'+'<textarea name="S1"  class="xlandinput" datasrc="#商品资料Tab" datafld="洗水唛信息" style="width:98%;height:96%" ></textarea></fieldset>\
        </div>';
    return s+s1+s2+s3;
}
...
function pubdata()
{
    ...
    var s = $("oListboxFrom").name +" in("+ getcusts($("oListboxFrom"))+") and 编码='"+gband.Val("编码")+"'";
...
function BeforeWinClose(wid)
{
    if(gband.IsModify())
    {
        var result = confirm("数据已改变,是否继续关闭窗口？");
        gband.Cancel();
