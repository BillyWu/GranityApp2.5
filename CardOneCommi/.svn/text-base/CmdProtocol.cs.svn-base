...
namespace Granity.CardOneCommi
{
    ...
    public class CmdProtocol : CommandBase
    {
        ...
        private ComClass comparse = new ComClass();
        ...
        public string FormatResponse(byte[] response)
        {
            ...
            try
            {
                info = comparse.AnalysisEateryResults(info, this.outterParam, this.dvParam, false, ref state);
            }
            catch
            {
                info = "@{状态}=响应解析错误,@Success=false";
            }
            return info;
        }
