...
namespace Estar.Common.Tools
{
	...
	public class UnitItem
	{
		...
		#region  公共属性
			...
			public	string				UnitName
			{
				...
				set
				{
                    ...
                    if (null == this._drnode || null == this._drnode.Table || DataRowState.Detached == this._drnode.RowState || !this._drnode.Table.Columns.Contains("name"))
                        return;
			...
			public	string				DataSrcFile
			{
				...
				set
				{
                    ...
                    if (null == this._drnode || null == this._drnode.Table || DataRowState.Detached == this._drnode.RowState || !this._drnode.Table.Columns.Contains("datasrcfile"))
                        return;
			...
			public	string				DictColSrcFile
			{
				...
				set
				{
                    ...
                    if (null == this._drnode || null == this._drnode.Table || DataRowState.Detached == this._drnode.RowState || !this._drnode.Table.Columns.Contains("dictcolfile"))
                        return;
        ...
        {
            get { return isCache; }
            set { isCache = value; if (!isCache)tabsCache.Clear(); }
        }
		#endregion
        ...
        {
            if (string.IsNullOrEmpty(sqltext) || string.IsNullOrEmpty(linkname))
                return null;
            string itemcode = "";
            if (null == this.dtWorkUnit || this.dtWorkUnit.Columns.Count < 1 || !this.dtWorkUnit.Columns.Contains("ntype"))
                return null;
            DataRow[] drs = this.dtWorkUnit.Select("ntype='" + linkname + "'");
            if (null == drs || drs.Length < 1)
                return null;
            for (int i = 0; i < drs.Length; i++)
                itemcode = itemcode + "'" + Convert.ToString(drs[i]["ID"]) + "',";
		...
		private	void SetWorkUnit(string	workUnit)
		{
            ...
            if (null == dtWorkUnit || dtWorkUnit.Columns.Count < 1)
        ...
        private void CreateXmlNode()
        {
            ...
            if (this.dtWorkItem != null)
            {
                ...
                for (int i = 0; i < drItem.Length; i++)
                {
                    ...
                    DataTable dtColumn = BindGrid(sqltext);
                    DataRow[] drcols=dtColumn.Select();
		...
		public AppendItem GetAppendItem(string	itemName)
		{
            ...
            DataTable tab    = BindGrid(sqltext);
		...
		{
            if (null == _drnode || null == this._drnode.Table || DataRowState.Detached == this._drnode.RowState || !this._drnode.Table.Columns.Contains("unitgroup"))
                return string.Empty;
            return Convert.ToString(this._drnode["unitgroup"]);
		}
