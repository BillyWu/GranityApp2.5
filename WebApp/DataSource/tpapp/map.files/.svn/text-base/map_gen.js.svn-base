		...
		if(refAdd.mtype!="frame")
		{
		    switch(xstate)
		    {
		        case "1":
		            ...
		            break;
		        case "2":  //设备正常,处于运行状态
		            refAdd.fillcolor="#2b5e13";
		            refAdd.itemcolor="#2b5e13";		        
		            if(document.getElementById(refAdd.id+"d"))
		            {
		                document.getElementById(refAdd.id+"d").strokecolor="#f0ec34";
                        document.getElementById(refAdd.id+"d").style.display="";
                    }
		            refAdd.fontcolor="black";
		            for(var j=0;j<refAdd.children.count;j++)
		                if(refAdd.children[j].tagName=="fill")
		                {
		                   refAdd.children[j].opacity="39321f"; 
		                   refAdd.children[j].color2="white";
		                   break;
		                }
    		        break;
		        case "3":  //设备正常,处于运行状态
		            refAdd.fillcolor="#4e7edb";
		            refAdd.itemcolor="#4e7edb";		        
		            if(document.getElementById(refAdd.id+"d"))
		            {
		                document.getElementById(refAdd.id+"d").strokecolor="#f0ec34";
                        document.getElementById(refAdd.id+"d").style.display="";
                ...
                case "9":	//煤气退火炉设备正常
                case "挤压":	//挤压机工作
                    if(refAdd.id.indexOf('newjy')>-1)
                    {
                        var refAdd1=document.getElementById('newjy0');
                        if(!refAdd1) break;
                        if(refAdd1)
                            document.getElementById('newjy0').fillcolor='green';                                       
                        break;
                    }              
		            refAdd.strokecolor="white";
		            refAdd.itemcolor="white";
		            break;	
                case "开电不挤压":	//煤气退火炉设备正常
                    if(refAdd.id.indexOf('newjy')>-1)
                    {
                        var refAdd1=document.getElementById('newjy0');
                        if(!refAdd1) break;
                        if(refAdd1)
                            document.getElementById('newjy0').fillcolor='#ff9900';                                       
                        break;
                    }              
		            break;		            	            
                case "无挤压":	//设备无信号
                    if(refAdd.id.indexOf('newjy')>-1)
                    {
                        ...
                        if(!refAdd1) break;
                        if(refAdd1)
                            document.getElementById('newjy0').fillcolor='gray';                                       
                        break;
                    }   
		            break;	
                case "0":	//设备无信号
		    ...
		    case "2":
		            if(!refAdd.baktext)
		            {
		                if(document.getElementById(refAdd.id+"td"))
		                    refAdd.setAttribute("baktext",document.getElementById(refAdd.id+"td").innerText);
		                else
		            ...
		            if(band.getFldStrValue("销量",i)!="")
		            {
		                if(!document.getElementById(refAdd.id+"td"))
		                {
		                    ...
		                    {		                    
		                        ...
		                        var sp ="";
		                        for(var k=0;k<refAdd.style.posWidth/21;k++)
		                            sp+=' ';
            		                
    		                    refAdd.children(0).children(0).children(0).children(0).children(0).innerText =sp+ refAdd.baktext+"     "+band.getFldStrValue("销量",i).replace(".00","");
    		                }
                            catch(ex)
                            {}
		                }
		                ...
		                {
		                    document.getElementById(refAdd.id+"td").style.fontsize="8";
...
function _onclick () 
{
	...
	if(oRubberBand.length>1) return;
	if (selectedGroup) {
...
function restorePosition (stack) 
{
	...
	{
	    document.all.tags("td").item(i).style.fontSize = document.all.tags("td").item(i).orifontSize;
	}
	
	for(var i=0;i<document.all.tags("img").length;i++)
	{
	    document.all.tags("img").item(i).width=document.all.tags("img").item(i).oriSize;
	}
...
function saveOriginalSizes (tagName)
{
	...
	for(var i=0;i<document.all.tags("td").length;i++)
	{
	    document.all.tags("td").item(i).setAttribute("orifontSize",document.all.tags("td").item(i).style.fontSize);
	}
	
	for(var i=0;i<document.all.tags("img").length;i++)
	{
	    document.all.tags("img").item(i).setAttribute("oriSize",document.all.tags("img").item(i).width);
	}
}
...
function scaleObject (object, scaleBy)
{
	...
	for(var i=0;i<document.all.tags("td").length;i++)
	{
	    var _xfont = document.all.tags("td").item(i).style.fontSize.replace("px","");
	    document.all.tags("td").item(i).style.fontSize = _xfont*scaleBy;
	}
	
	for(var i=0;i<document.all.tags("img").length;i++)
	{
	    var _xfont = document.all.tags("img").item(i).width;
	    document.all.tags("img").item(i).width = _xfont*scaleBy;
	}
}
