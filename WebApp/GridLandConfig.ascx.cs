...
namespace Estar.WebApp
{
    ...
    ///		具有分页功能的Grid工作项目控件
    /// </summary>
    public partial class GridLandConfig : BaseControl
    {
        //控件显示的容器,脚本的容器
        ...
        //控件模板文件,默认一个文件
        private string _tpGridFile = "ControlTemplate/GridDefault.htm";
        //数据岛的所有字典数据集
        ...
        {
            XmlNode xmlTabTitle = this._xmltpdoc.SelectSingleNode("//*[@rowType='title']");
            ...
            xmlDivTitle.Attributes["style"].Value += ";width:" + this.GridDiv.Width.Value.ToString();
        }
        ...
        private void setDetail()
        {
            ...
            //建立的明细区的列单元格
            for (int i = 0; i < this.CtrlDataSource.Columns.Count; i++)
                this.setCol(this.CtrlDataSource.Columns[i], xmlRowDetail, xmlTdNext);
            ...
            xmlDivDetail.Attributes["style"].Value += ";width:" + this.GridDiv.Width.Value.ToString();
        }
        ...
        private XmlNode setCol(DataColumn col, XmlNode xmlRow, XmlNode xmlTdNext)
        {
        ...
        private void setFoot()
        {
            ...
            for (int i = 0; i < this.CtrlDataSource.Columns.Count; i++)
            {
                ...
                if ("int" != strType && "decimal" != strType)
                    continue;
                if (null == xmlFootLb.Attributes["datafld"])
                    xmlFootLb.Attributes.Append(xmlFootLb.OwnerDocument.CreateAttribute("datafld"));
                XmlAttribute attrFldFormat = xmlCol.Attributes["formatfld"];
                xmlFootLb.Attributes["datafld"].Value = (null == attrFldFormat || "" == attrFldFormat.Value) ? colName : attrFldFormat.Value;
        ...
        /// <summary>
        /// 把配置信息填充到tab中,tab的结构自动建立
        /// tab结构:配置类别,名称,配置值,数据库类别
        /// 配置类别://目前有两种:DataSource,appSettings
        /// 数据库类别://目前有三种:OdpNet,OleDb,SqlClient
        /// </summary>
        /// <param name="tab">需要填充的表格</param>
        public void FillTabConfig(DataTable tab)
        {
            KeyValueConfigurationCollection     settings=DataAccRes.DefaultConfiguration.AppSettings.Settings;
            int icount = settings.Count;
            for (int i = 0; i < icount; i++)
            {
                string strname = settings.AllKeys[i];
                DataRow[] drs = tab.Select("名称='" + strname +"'");
                DataRow dr = null;
                if (null == drs || drs.Length < 1)
                {
                    ...
                    dr = drs[0];
                dr["ID"] = (null == dr["ID"] || DBNull.Value==dr["ID"]) ? Guid.NewGuid() : dr["ID"];
                dr["名称"] = settings.AllKeys[i];
                dr["配置值"] = settings[settings.AllKeys[i]].Value;
                dr["配置类别"] = "appSettings";
            }
            DataConnSection conn = DataAccRes.DefaultSection;
            foreach (DataConnInfo dc in conn.DataConnList)
            {
